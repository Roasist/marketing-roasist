...
function calculateOfficialLocationBreakdown($apiKeys, $query, $mode, $officialKeywords, $geoConstants, $langCode = 'tr', $locationsMeta = []) {
    ...
    // Build all requests using generateKeywordHistoricalMetrics (batches of 1000 keywords for lightning fast response)
    $allRequests = [];
    $diagnostics = []; // Per-location debug info

    if (!empty($topSeeds)) {
        $seedBatches = array_chunk($topSeeds, 1000); // 1000 keywords per batch
        foreach ($geoConstants as $geo) {
            $geoResource = strpos($geo, 'geoTargetConstants/') === 0 ? $geo : "geoTargetConstants/{$geo}";
            $geoId = preg_replace('/[^0-9]/', '', $geo);
            foreach ($seedBatches as $seedList) {
    ...
    // Process requests in batches of 14 concurrent requests
    ...
    foreach ($requestBatches as $batchIdx => $batch) {
        ...
        foreach ($batch as $reqIdx => $req) {
            ...
            curl_setopt($chLoc, CURLOPT_POSTFIELDS, json_encode($req['payload']));
            curl_setopt($chLoc, CURLOPT_RETURNTRANSFER, true);
            curl_setopt($chLoc, CURLOPT_TIMEOUT, 20);
            curl_setopt($chLoc, CURLOPT_HTTPHEADER, [
                "Authorization: Bearer {$accessToken}",
                "developer-token: {$devToken}",
                "Content-Type: application/json"
        ...
        foreach ($curlHandles as $hData) {
            ...
            if ($httpCode === 429 || $httpCode === 503 || $httpCode === 500) {
                $failedRequests[] = $req;
                $diagnostics[$geoId]['errors'][] = "HTTP {$httpCode} — queued for retry";
                continue;
            }
            ...
            foreach ($results as $r) {
                ...
                if (!empty($kwText)) {
                    ...
                    $geoAliases = [
                        '9070443' => ['1009828', '9258772'],
                        '1009828' => ['9070443', '9258772'],
                        '9258772' => ['9070443', '1009828'],
                        '9263995' => ['1009827', '9070440'],
                        '1009827' => ['9263995', '9070440'],
                        '9070438' => ['9250286', '9250593'],
                        '9075982' => ['1028523'],
                        '1028523' => ['9075982'],
                        '9075980' => ['1028524'],
                        '1028524' => ['9075980'],
                        '9075977' => ['1028525'],
                        '1028525' => ['9075977'],
                        '9070290' => ['9063099', '9235214'],
                        '9063099' => ['9070290', '9235214'],
                        '1009806' => ['20400'],
                        '20400'   => ['1009806'],
                        '9070292' => ['1009805', '20399'],
                        '1009805' => ['9070292', '20399'],
                        '1009804' => ['20398'],
                        '20398'   => ['1009804'],
                        '21125'   => ['1012866'],
                        '1012866' => ['21125'],
                    ...
                    $allMatchingGeos = array_merge([(string)$geoId], $geoAliases[(string)$geoId] ?? []);
                    foreach ($allMatchingGeos as $mGeo) {
                        $keywordGeoMap[$kwNorm][$mGeo] = $geoEntry;
                        $keywordGeoMap[$kwRaw][$mGeo] = $geoEntry;
                    }
                }
    ...
    // Calculate input keywords baseline average CPC if available
    $inputKeywordCpcSum = 0;
    $inputKeywordCpcCount = 0;
    if (!empty($officialKeywords) && is_array($officialKeywords)) {
        foreach ($officialKeywords as $okw) {
            if (is_array($okw)) {
                $low = (float)($okw['lowCpc'] ?? 0);
                $high = (float)($okw['highCpc'] ?? 0);
                if ($high > 0) {
                    $inputKeywordCpcSum += ($low + $high) / 2;
                    $inputKeywordCpcCount++;
                }
            }
        }
    }
...
function fetchGoogleAdsOfficialKeywordIdeas($apiKeys, $url, $keywords, $langCode = 'tr', $countryCode = 'TR', $geoTargetConstants = []) {
    ...
    $parseLocationResults = function($parallelResults) use (&$parsedKeywords, &$keywordIndexMap, $seedKeys) {
        foreach ($parallelResults as $item) {
            ...
            foreach ($adsJson['results'] as $idx => $r) {
                ...
                $metrics = $r['keywordIdeaMetrics'] ?? $r['keywordMetrics'] ?? [];
                ...
                if (isset($keywordIndexMap[$kwKey])) {
                    ...
                    if ($geoId === 'ALL') {
                        ...
                        if ($parsedKeywords[$pos]['monthlyVolume'] === 0 && $avgVol > 0) {
                            $parsedKeywords[$pos]['monthlyVolume'] = $avgVol;
                        }
                    }
                    continue;
                ...
                $negativeJunkRentalPattern = '/(?:^|[^\p{L}\p{N}])('
                    ...
                    . '|bedava|ücretsiz|free|مجانی|رayگان|бесплатно|kostenlos'
                ...
                $geoCpcInit = [];
                if ($geoId !== 'ALL') {
                    $geoVolumesInit[$geoId] = $avgVol;
                    $geoCpcInit[$geoId] = ['lowCpc' => $lowBid, 'highCpc' => $highBid];
                }
    ...
    // Step C: Query individual location breakdown for top discovered keywords across all locations
...
function generateSemanticKeywordsFallback($query, $pageDetails, $langCode) {
    ...
    if ($langCode === 'ru' || preg_match('/(grazhdanstv|nedvizhim|kvartir|turtsiy|pasport|vnzh|23projects|23square)/i', $full)) {
        return [
            ...
            'keywords' => [
                ['keyword' => 'гражданство Турции за инвестиции', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'HIGH', 'competitionIndex' => 85, 'intent' => 'TRANSACTIONAL', 'trendChangePercent' => 25, 'opportunityScore' => 95],
                ['keyword' => 'турецкое гражданство при покупке недвижимости', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'HIGH', 'competitionIndex' => 82, 'intent' => 'TRANSACTIONAL', 'trendChangePercent' => 20, 'opportunityScore' => 93],
                ['keyword' => 'купить квартиру в Аланье для гражданства', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'HIGH', 'competitionIndex' => 78, 'intent' => 'TRANSACTIONAL', 'trendChangePercent' => 15, 'opportunityScore' => 92],
                ['keyword' => 'недвижимость в Турции паспорт', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'MEDIUM', 'competitionIndex' => 70, 'intent' => 'COMMERCIAL', 'trendChangePercent' => 18, 'opportunityScore' => 88],
                ['keyword' => 'получить паспорт Турции гражданину РФ', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'HIGH', 'competitionIndex' => 80, 'intent' => 'TRANSACTIONAL', 'trendChangePercent' => 30, 'opportunityScore' => 94],
                ['keyword' => 'пакет для гражданства 23 square', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'LOW', 'competitionIndex' => 45, 'intent' => 'TRANSACTIONAL', 'trendChangePercent' => 40, 'opportunityScore' => 96],
                ['keyword' => 'инвестиции в недвижимость Турции 2026', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'MEDIUM', 'competitionIndex' => 65, 'intent' => 'COMMERCIAL', 'trendChangePercent' => 12, 'opportunityScore' => 86],
                ['keyword' => 'квартиры от застройщика Аланья гражданство', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'HIGH', 'competitionIndex' => 76, 'intent' => 'TRANSACTIONAL', 'trendChangePercent' => 22, 'opportunityScore' => 91],
                ['keyword' => 'минимальная сумма для гражданства Турции', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'MEDIUM', 'competitionIndex' => 60, 'intent' => 'INFORMATIONAL', 'trendChangePercent' => 10, 'opportunityScore' => 84],
                ['keyword' => 'второе гражданство Турция недвижимость', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'HIGH', 'competitionIndex' => 74, 'intent' => 'TRANSACTIONAL', 'trendChangePercent' => 16, 'opportunityScore' => 89],
                ['keyword' => 'элитное жилье в Турции под паспорт', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'HIGH', 'competitionIndex' => 79, 'intent' => 'TRANSACTIONAL', 'trendChangePercent' => 19, 'opportunityScore' => 90],
                ['keyword' => 'оформление турецкого гражданства под ключ', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'HIGH', 'competitionIndex' => 83, 'intent' => 'TRANSACTIONAL', 'trendChangePercent' => 28, 'opportunityScore' => 92],
                ['keyword' => 'апартаменты в Аланье у моря', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'MEDIUM', 'competitionIndex' => 68, 'intent' => 'COMMERCIAL', 'trendChangePercent' => 14, 'opportunityScore' => 85],
                ['keyword' => 'доходная недвижимость в Турции', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'MEDIUM', 'competitionIndex' => 66, 'intent' => 'COMMERCIAL', 'trendChangePercent' => 15, 'opportunityScore' => 87],
                ['keyword' => 'купить виллу в Аланье гражданство', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'HIGH', 'competitionIndex' => 81, 'intent' => 'TRANSACTIONAL', 'trendChangePercent' => 21, 'opportunityScore' => 91],
                ['keyword' => 'программа гражданства Турции через недвижимость', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'HIGH', 'competitionIndex' => 77, 'intent' => 'TRANSACTIONAL', 'trendChangePercent' => 17, 'opportunityScore' => 89],
                ['keyword' => 'сроки получения турецкого паспорта при покупке жилья', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'MEDIUM', 'competitionIndex' => 58, 'intent' => 'INFORMATIONAL', 'trendChangePercent' => 11, 'opportunityScore' => 82],
                ['keyword' => 'надежный застройщик в Аланье Турция', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'MEDIUM', 'competitionIndex' => 62, 'intent' => 'COMMERCIAL', 'trendChangePercent' => 13, 'opportunityScore' => 86],
                ['keyword' => 'купить недвижимость в Аланье недорого', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'HIGH', 'competitionIndex' => 75, 'intent' => 'TRANSACTIONAL', 'trendChangePercent' => 16, 'opportunityScore' => 90],
                ['keyword' => 'турецкий паспорт за инвестиции 400000', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'HIGH', 'competitionIndex' => 84, 'intent' => 'TRANSACTIONAL', 'trendChangePercent' => 24, 'opportunityScore' => 93],
                ['keyword' => 'купить квартиру в Анталии у моря', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'HIGH', 'competitionIndex' => 79, 'intent' => 'TRANSACTIONAL', 'trendChangePercent' => 18, 'opportunityScore' => 91],
                ['keyword' => 'внж в Турции при покупке квартиры 2026', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'MEDIUM', 'competitionIndex' => 65, 'intent' => 'TRANSACTIONAL', 'trendChangePercent' => 26, 'opportunityScore' => 88],
                ['keyword' => 'инвестиции в апартаменты Турция доход', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'MEDIUM', 'competitionIndex' => 69, 'intent' => 'COMMERCIAL', 'trendChangePercent' => 14, 'opportunityScore' => 86],
                ['keyword' => 'пентхаус в Аланье с видом на море', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'HIGH', 'competitionIndex' => 76, 'intent' => 'COMMERCIAL', 'trendChangePercent' => 12, 'opportunityScore' => 85],
                ['keyword' => 'покупка недвижимости в Турции пошагово', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'LOW', 'competitionIndex' => 50, 'intent' => 'INFORMATIONAL', 'trendChangePercent' => 10, 'opportunityScore' => 80],
                ['keyword' => 'налоги при покупке недвижимости в Турции', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'LOW', 'competitionIndex' => 52, 'intent' => 'INFORMATIONAL', 'trendChangePercent' => 12, 'opportunityScore' => 81],
                ['keyword' => 'жилые комплексы в Аланье с инфраструктурой', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'MEDIUM', 'competitionIndex' => 67, 'intent' => 'COMMERCIAL', 'trendChangePercent' => 19, 'opportunityScore' => 87],
                ['keyword' => 'купить таунхаус в Турции гражданство', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'HIGH', 'competitionIndex' => 80, 'intent' => 'TRANSACTIONAL', 'trendChangePercent' => 15, 'opportunityScore' => 89],
                ['keyword' => 'открытие счета в Турции для покупки жилья', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'LOW', 'competitionIndex' => 48, 'intent' => 'INFORMATIONAL', 'trendChangePercent' => 18, 'opportunityScore' => 83],
                ['keyword' => 'недвижимость в Стамбуле под гражданство', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'HIGH', 'competitionIndex' => 87, 'intent' => 'TRANSACTIONAL', 'trendChangePercent' => 22, 'opportunityScore' => 92],
                ['keyword' => 'проверенное агентство недвижимости в Аланье', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'MEDIUM', 'competitionIndex' => 64, 'intent' => 'COMMERCIAL', 'trendChangePercent' => 14, 'opportunityScore' => 86],
                ['keyword' => 'новостройки в Аланье рассрочка 0%', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'HIGH', 'competitionIndex' => 78, 'intent' => 'TRANSACTIONAL', 'trendChangePercent' => 20, 'opportunityScore' => 90],
                ['keyword' => 'экспертиза недвижимости в Турции экспертиз рапор', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'LOW', 'competitionIndex' => 46, 'intent' => 'INFORMATIONAL', 'trendChangePercent' => 10, 'opportunityScore' => 81],
                ['keyword' => 'купить 1+1 в Аланье для сдачи в аренду', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'HIGH', 'competitionIndex' => 74, 'intent' => 'TRANSACTIONAL', 'trendChangePercent' => 17, 'opportunityScore' => 89],
                ['keyword' => 'гражданство Турции для всей семьи за инвестиции', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'HIGH', 'competitionIndex' => 83, 'intent' => 'TRANSACTIONAL', 'trendChangePercent' => 25, 'opportunityScore' => 93],
                ['keyword' => 'купить апартаменты в Турции у застройщика', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'HIGH', 'competitionIndex' => 77, 'intent' => 'TRANSACTIONAL', 'trendChangePercent' => 18, 'opportunityScore' => 91],
                ['keyword' => 'инвестиции в недвижимость Турции гражданам Казахстана', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'MEDIUM', 'competitionIndex' => 70, 'intent' => 'TRANSACTIONAL', 'trendChangePercent' => 22, 'opportunityScore' => 90],
                ['keyword' => 'инвестиции в недвижимость Турции гражданам Узбекистана', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'MEDIUM', 'competitionIndex' => 68, 'intent' => 'TRANSACTIONAL', 'trendChangePercent' => 20, 'opportunityScore' => 89],
                ['keyword' => 'гражданство Турции для граждан Кыргызстана', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'MEDIUM', 'competitionIndex' => 65, 'intent' => 'TRANSACTIONAL', 'trendChangePercent' => 19, 'opportunityScore' => 88],
                ['keyword' => 'купить квартиру в Махмутларе Аланья', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'HIGH', 'competitionIndex' => 79, 'intent' => 'TRANSACTIONAL', 'trendChangePercent' => 16, 'opportunityScore' => 90],
                ['keyword' => 'квартиры в Оба Аланья от застройщика', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'HIGH', 'competitionIndex' => 75, 'intent' => 'TRANSACTIONAL', 'trendChangePercent' => 15, 'opportunityScore' => 89],
                ['keyword' => 'квартира в Каргыджаке элитный комплекс', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'HIGH', 'competitionIndex' => 76, 'intent' => 'COMMERCIAL', 'trendChangePercent' => 14, 'opportunityScore' => 87],
                ['keyword' => 'оформление ТАПУ в Турции иностранцу', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'LOW', 'competitionIndex' => 50, 'intent' => 'INFORMATIONAL', 'trendChangePercent' => 12, 'opportunityScore' => 83],
                ['keyword' => 'купить дом на побережье Турции', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'HIGH', 'competitionIndex' => 81, 'intent' => 'TRANSACTIONAL', 'trendChangePercent' => 17, 'opportunityScore' => 91],
                ['keyword' => 'квартиры в Мерсине у моря недорого', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'HIGH', 'competitionIndex' => 78, 'intent' => 'TRANSACTIONAL', 'trendChangePercent' => 28, 'opportunityScore' => 92],
                ['keyword' => 'ВНЖ в Мерсине при покупке квартиры', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'MEDIUM', 'competitionIndex' => 66, 'intent' => 'TRANSACTIONAL', 'trendChangePercent' => 25, 'opportunityScore' => 89],
                ['keyword' => 'элитные виллы в Бодруме под гражданство', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'HIGH', 'competitionIndex' => 85, 'intent' => 'TRANSACTIONAL', 'trendChangePercent' => 20, 'opportunityScore' => 93],
                ['keyword' => 'паспорт Турции без проживания в стране', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'HIGH', 'competitionIndex' => 82, 'intent' => 'TRANSACTIONAL', 'trendChangePercent' => 32, 'opportunityScore' => 94],
                ['keyword' => 'как получить гражданство Турции через недвижимость', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'MEDIUM', 'competitionIndex' => 59, 'intent' => 'INFORMATIONAL', 'trendChangePercent' => 15, 'opportunityScore' => 85],
                ['keyword' => 'доход от аренды квартир в Аланье 2026', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'MEDIUM', 'competitionIndex' => 61, 'intent' => 'COMMERCIAL', 'trendChangePercent' => 14, 'opportunityScore' => 84],
                ['keyword' => 'купить квартиру в Турции без посредников напрямую', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'HIGH', 'competitionIndex' => 74, 'intent' => 'TRANSACTIONAL', 'trendChangePercent' => 19, 'opportunityScore' => 90],
                ['keyword' => 'недвижимость в Турции рассрочка от застройщика', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'HIGH', 'competitionIndex' => 77, 'intent' => 'TRANSACTIONAL', 'trendChangePercent' => 21, 'opportunityScore' => 91]
            ]
        ];
    }
    ...
    if ($isMarketing) {
        return [
            ...
            'keywords' => [
                ['keyword' => 'performans pazarlama ajansı', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'HIGH', 'competitionIndex' => 88, 'intent' => 'TRANSACTIONAL', 'trendChangePercent' => 22, 'opportunityScore' => 93],
                ['keyword' => 'google ads reklam yönetimi ajansı', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'HIGH', 'competitionIndex' => 92, 'intent' => 'TRANSACTIONAL', 'trendChangePercent' => 18, 'opportunityScore' => 95],
                ['keyword' => 'meta reklam danışmanlığı', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'HIGH', 'competitionIndex' => 85, 'intent' => 'TRANSACTIONAL', 'trendChangePercent' => 25, 'opportunityScore' => 91],
                ['keyword' => 'e-ticaret roas artırma yöntemleri', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'MEDIUM', 'competitionIndex' => 70, 'intent' => 'COMMERCIAL', 'trendChangePercent' => 30, 'opportunityScore' => 89],
                ['keyword' => 'dijital pazarlama ajansı istanbul', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'HIGH', 'competitionIndex' => 90, 'intent' => 'TRANSACTIONAL', 'trendChangePercent' => 15, 'opportunityScore' => 92],
                ['keyword' => 'facebook reklam hesabı optimizasyonu', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'MEDIUM', 'competitionIndex' => 68, 'intent' => 'COMMERCIAL', 'trendChangePercent' => 16, 'opportunityScore' => 86],
                ['keyword' => 'tiktok reklam ajansı türkiye', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'HIGH', 'competitionIndex' => 79, 'intent' => 'TRANSACTIONAL', 'trendChangePercent' => 35, 'opportunityScore' => 94],
                ['keyword' => 'profesyonel google ads danışmanı', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'HIGH', 'competitionIndex' => 84, 'intent' => 'TRANSACTIONAL', 'trendChangePercent' => 20, 'opportunityScore' => 90],
                ['keyword' => 'reklam bütçesi yönetimi ve optimizasyon', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'MEDIUM', 'competitionIndex' => 62, 'intent' => 'COMMERCIAL', 'trendChangePercent' => 14, 'opportunityScore' => 84],
                ['keyword' => 'e-ticaret büyüme ajansı', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'HIGH', 'competitionIndex' => 81, 'intent' => 'TRANSACTIONAL', 'trendChangePercent' => 28, 'opportunityScore' => 89],
                ['keyword' => 'reklam kreatif optimizasyonu', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'MEDIUM', 'competitionIndex' => 58, 'intent' => 'COMMERCIAL', 'trendChangePercent' => 24, 'opportunityScore' => 85],
                ['keyword' => 'dijital reklam ajansı fiyatları', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'HIGH', 'competitionIndex' => 83, 'intent' => 'TRANSACTIONAL', 'trendChangePercent' => 12, 'opportunityScore' => 88]
            ]
        ];
    }
    ...
    if ($isRealEstateTr) {
        return [
            ...
            'keywords' => [
                ['keyword' => 'alanya satılık daire denize sıfır', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'HIGH', 'competitionIndex' => 86, 'intent' => 'TRANSACTIONAL', 'trendChangePercent' => 20, 'opportunityScore' => 92],
                ['keyword' => 'antalya satılık lüks villa', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'HIGH', 'competitionIndex' => 88, 'intent' => 'TRANSACTIONAL', 'trendChangePercent' => 18, 'opportunityScore' => 94],
                ['keyword' => 'alanya emlak projeleri lansman', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'MEDIUM', 'competitionIndex' => 74, 'intent' => 'COMMERCIAL', 'trendChangePercent' => 25, 'opportunityScore' => 89],
                ['keyword' => 'yabancıya konut satışı vatandaşlık', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'HIGH', 'competitionIndex' => 84, 'intent' => 'TRANSACTIONAL', 'trendChangePercent' => 22, 'opportunityScore' => 91],
                ['keyword' => 'alanya mahmutlar satılık daire', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'HIGH', 'competitionIndex' => 80, 'intent' => 'TRANSACTIONAL', 'trendChangePercent' => 15, 'opportunityScore' => 88],
                ['keyword' => 'türkiye gayrimenkul yatırım getirisi', 'monthlyVolume' => 0, 'lowCpc' => 0.0, 'highCpc' => 0.0, 'competition' => 'MEDIUM', 'competitionIndex' => 68, 'intent' => 'COMMERCIAL', 'trendChangePercent' => 14, 'opportunityScore' => 85]
            ]
        ];
    }
