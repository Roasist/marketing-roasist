...
export const CORE_GEO_ENTITIES: Record<string, GeoTargetLocation> = {
  ...
  UA_ODESA: { id: '1012854', resourceName: 'geoTargetConstants/1012854', name: 'Odessa (Odesa)', canonicalName: 'Odesa, Ukraine', countryCode: 'UA', targetType: 'City', reach: 1000000, flag: '🇺🇦', cpcMultiplier: 1.0, volumeMultiplier: 1.0 },
...
export const GEO_ALIASES_MAP: Record<string, string[]> = {
  // Kyrgyzstan
  '9070443': ['1009828', '9258772', '9070443'], // Osh Region <-> Osh City
  '1009828': ['9070443', '9258772', '1009828'], // Osh City <-> Osh Region
  '9258772': ['9070443', '1009828', '9258772'],
  '9263995': ['1009827', '9070440', '9263995'], // Bishkek City <-> Bishkek
  '1009827': ['9263995', '9070440', '1009827'],
  '9070438': ['9250286', '9250593', '9070438'], // Jalal-Abad Region <-> Cities
  '9250286': ['9070438', '9250286'],
  // Uzbekistan
  '9075982': ['1028523', '9075982'], // Samarqand Region <-> City
  '1028523': ['9075982', '1028523'],
  '9075980': ['1028524', '9075980'], // Fergana Region <-> City
  '1028524': ['9075980', '1028524'],
  '9075977': ['1028525', '9075977'], // Namangan Region <-> City
  '1028525': ['9075977', '1028525'],
  '1028531': ['20836', '1028308', '1028531'], // Tashkent
  '20836': ['1028531', '1028308', '20836'],
  '1028308': ['20836', '1028531', '1028308'],
  '20835': ['1028310', '20835'],
  '20833': ['1028311', '20833'],
  '20834': ['1028309', '20834'],
  // Kazakhstan
  '9070290': ['9063099', '9235214', '9070290'], // Almaty Region <-> City
  '9063099': ['9070290', '9235214', '9063099'],
  '9235214': ['9063099', '9070290', '9235214'],
  '1009806': ['20400', '1009806'], // Astana / Nur-Sultan
  '20400': ['1009806', '20400'],
  '9070292': ['1009805', '20399', '9070292'], // Karaganda Region <-> City
  '1009805': ['9070292', '20399', '1009805'],
  '20399': ['9070292', '1009805', '20399'],
  '1009804': ['20398', '1009804'], // Shymkent
  '20398': ['1009804', '20398'],
  // Ukraine
  '21125': ['1012866', '21125'], // Kharkiv Oblast <-> City
  '1012866': ['21125', '1012866'],
  '21118': ['21119', '21118'], // Kyiv City <-> Oblast
  '21119': ['21118', '21119'],
  '21122': ['1012856', '21122'], // Odesa Oblast <-> City
  '1012856': ['21122', '1012856'],
  // Turkey Provinces <-> Cities <-> Districts
  '21069': ['1012782', '21069'], // Istanbul Province <-> City
  '1012782': ['21069', '1012782'],
  '21055': ['1012763', '21055'], // Ankara Province <-> City
  '1012763': ['21055', '1012763'],
  '21056': ['1012764', '9056714', '9225632', '21056'], // Antalya Province <-> City <-> Alanya
  '1012764': ['21056', '9056714', '9225632', '1012764'],
  '21060': ['1012769', '21060'], // Bursa Province <-> City
  '1012769': ['21060', '1012769'],
  '21094': ['9056898', '21094'], // Yalova Province <-> City
  '9056898': ['21094', '9056898'],
  '21070': ['1012783', '21070'], // Izmir Province <-> City
  '1012783': ['21070', '1012783'],
  '21079': ['1012792', '9056736', '9056778', '9056829', '21079'], // Mugla Province <-> Bodrum <-> Fethiye <-> Marmaris
  '1012792': ['21079', '9056736', '9056778', '9056829', '1012792'],
  '9056736': ['21079', '1012792', '9056736'],
...
export const ForecastModule: React.FC<ForecastModuleProps> = ({ workspaceId }) => {
  ...
  const [locationAuditFilterQuery, setLocationAuditFilterQuery] = useState<string>('');
  ...
  const handleDiscover = async (customQuery?: string, customMode?: 'URL' | 'KEYWORDS', bypassCache: boolean = true) => {
    ...
    try {
      ...
      if (res && res.keywords && res.keywords.length > 0) {
        ...
        const auditItems = activeLocations.map((loc: any) => {
          ...
          for (const k of finalKeywords) {
            ...
            if (k.geoVolumes) {
              ...
              for (const kt of keysToTry) {
                if (k.geoVolumes[kt] !== undefined) {
                  v = Number(k.geoVolumes[kt]) || 0;
                  break;
                }
              }
            }
          ...
          return {
            ...
            targetType: loc.targetType || 'City',
            flag: loc.flag || '🌍',
            searchVolume: volSum,
        ...
        const auditSummary = {
          locations: auditItems,
          totalLocations: activeLocations.length,
          successLocations: auditItems.filter(i => i.searchVolume > 0).length,
          totalVolume: finalKeywords.reduce((acc, k) => acc + (k.monthlyVolume || 0), 0),
          totalKeywords: finalKeywords.length,
          detectedLanguage: res.detectedLanguage || 'tr',
          sector: res.sector || 'Genel'
        };

        setLocationAuditData(auditSummary);
  ...
  const countryBreakdown: CountryMetric[] = useMemo(() => {
    ...
    for (const loc of selectedLocations) {
      ...
      for (const k of activePool) {
        let kwVol = 0;
        if (k.geoVolumes && Object.keys(k.geoVolumes).length > 0) {
          const extraAliases = GEO_ALIASES_MAP[cleanId] || [];
          const keysToTry = [
            cleanId,
            locKey,
            ...extraAliases,
  ...
  const scopedKeywords = useMemo(() => {
    ...
    return imputedKeywords.map(k => {
      ...
      if (k.geoVolumes && Object.keys(k.geoVolumes).length > 0) {
        const extraAliases = GEO_ALIASES_MAP[cleanGeoId] || [];
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                ...
                <div>
                  ...
                  <p style={{ fontSize: '0.78rem', color: 'var(--text-secondary)', margin: 0 }}>
                    Master Planınız altına platformu ve hedefleme modelini belirleyin.
                  </p>
                </div>
              </div>
              <button 
      ...
      {keywords.length > 0 && (
        <div className="card" style={{ padding: '0.6rem 0.85rem', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '0.65rem', backgroundColor: 'var(--bg-surface)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <div className="card" style={{ padding: '0.75rem 1.1rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '0.75rem', backgroundColor: 'var(--bg-surface-elevated)', border: '1px solid var(--border-default)' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap' }}>
                  <button
                    type="button"
                    onClick={() => setShowLocationAuditModal(true)}
                    className="btn-ghost"
                    style={{ fontSize: '0.78rem', padding: '0.35rem 0.75rem', border: '1px solid rgba(16, 185, 129, 0.35)', backgroundColor: 'rgba(16, 185, 129, 0.08)', borderRadius: 'var(--radius-xs)', display: 'flex', alignItems: 'center', gap: '0.4rem', color: '#10b981', fontWeight: 600 }}
                  >
                    <Activity size={13} />
                    <span>📍 Google Ads API Raporu</span>
                  </button>
          ...
          {/* ========================================================================= */}
          {/* CHANNEL: GOOGLE DEMAND GEN (SHORTS, DISCOVER & GMAIL)                     */}
          {/* ========================================================================= */}
          {activeChannelTab === 'DEMAND_GEN' && (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(360px, 1fr))', gap: '1.25rem' }}>
              ...
              <div className="card" style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                ...
                {/* Healthy Lead Rate Slider */}
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <label style={{ fontSize: '0.825rem', fontWeight: 600, color: 'var(--text-primary)' }}>
                      Nitelikli / Sağlıklı Lead Oranı (%)
                    </label>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '3px' }}>
                      <input
                        type="number"
              ...
              {/* Right Column: Demand Gen Output 4 Core KPI Cards */}
              <div className="card" style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.85rem' }}>
                  ...
                  <div style={{ padding: '1rem', backgroundColor: 'var(--bg-surface-elevated)', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border-default)' }}>
                    <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)', fontWeight: 600, textTransform: 'uppercase' }}>Toplam Gösterim & Erişim</div>
                    ...
                    <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginTop: '4px' }}>
                      Ort. CPM: ₺{demandGenSimulation.cpm}
                    </div>
                  </div>

                  <div style={{ padding: '1rem', backgroundColor: 'var(--bg-surface-elevated)', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border-default)' }}>
                    <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)', fontWeight: 600, textTransform: 'uppercase' }}>Tıklama & Ziyaretçi</div>
                    <div style={{ fontSize: '1.45rem', fontWeight: 800, color: '#3b82f6', marginTop: '4px' }}>
                      {demandGenSimulation.clicks.toLocaleString('tr-TR')}
                    </div>
                    <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginTop: '4px' }}>
                      Ort. TBM: ₺{demandGenSimulation.cpc}
                    </div>
                  </div>
                  ...
                  <div style={{ padding: '1rem', backgroundColor: 'var(--bg-surface-elevated)', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border-default)' }}>
                    ...
                    <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginTop: '4px' }}>
                      Talep Başı Maliyet (CPL): ₺{demandGenSimulation.cpl}
                    </div>
                  </div>

                  <div style={{ padding: '1rem', backgroundColor: 'var(--bg-surface-elevated)', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border-default)' }}>
                    <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)', fontWeight: 600, textTransform: 'uppercase' }}>Sağlıklı / Nitelikli Lead (MQL)</div>
                    <div style={{ fontSize: '1.45rem', fontWeight: 800, color: '#10b981', marginTop: '4px' }}>
                      {demandGenSimulation.healthyLeads.toLocaleString('tr-TR')}
                    </div>
                    <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginTop: '4px' }}>
                      Nitelikli Lead Başı Maliyet (CPQL): ₺{demandGenSimulation.cpql}
                    </div>
                  </div>
      ...
      {/* 📍 Google Ads API Location & Volume Audit Inspection Modal */}
      {showLocationAuditModal && (
        <div
          style={{
            position: 'fixed',
            inset: 0,
        ...
        >
          ...
          >
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', borderBottom: '1px solid var(--border-default)', paddingBottom: '1rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                <div style={{ padding: '0.65rem', borderRadius: 'var(--radius-sm)', backgroundColor: 'rgba(37, 99, 235, 0.15)', color: 'var(--brand-primary)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <Globe size={22} />
                </div>
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap' }}>
                    <h3 style={{ margin: 0, fontSize: '1.15rem', fontWeight: 700, color: 'var(--text-primary)' }}>
                      Google Ads API — Lokasyon & Hacim Doğrulama Raporu
                    </h3>
                    <span className="badge badge-active" style={{ background: 'rgba(16, 185, 129, 0.15)', color: '#059669', border: '1px solid rgba(16, 185, 129, 0.3)', fontSize: '0.7rem', fontWeight: 600 }}>
                      <CheckCircle size={10} style={{ marginRight: 3 }} /> %100 Resmi Google API Verisi
                    </span>
                  </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '0.75rem' }}>
              <div style={{ padding: '0.85rem 1rem', backgroundColor: 'var(--bg-surface-elevated)', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border-default)' }}>
                ...
                <div style={{ fontSize: '1.25rem', fontWeight: 700, color: 'var(--text-primary)', marginTop: '0.2rem' }}>
                  {locationAuditData?.successLocations || 0} / {locationAuditData?.totalLocations || selectedLocations.length} Aktif
                </div>
                <div style={{ fontSize: '0.7rem', color: (locationAuditData?.successLocations === locationAuditData?.totalLocations) ? '#10b981' : '#f59e0b', marginTop: '0.2rem', display: 'flex', alignItems: 'center', gap: '3px' }}>
                  {locationAuditData?.successLocations === locationAuditData?.totalLocations ? '✓ Tüm bölgelerden veri alındı' : '⚠️ Bazı bölgelerde 0 hacim'}
                </div>
              </div>
              ...
              <div style={{ padding: '0.85rem 1rem', backgroundColor: 'var(--bg-surface-elevated)', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border-default)' }}>
                ...
                <div style={{ fontSize: '1.15rem', fontWeight: 700, color: '#10b981', marginTop: '0.2rem', display: 'flex', alignItems: 'center', gap: '4px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '0.75rem', flexWrap: 'wrap' }}>
              <div style={{ position: 'relative', flex: 1, minWidth: '240px' }}>
                ...
                <input
                  type="text"
                  placeholder="Lokasyon veya ID ara (örn: Almatı, Odesa, 1009804)..."
                  value={locationAuditFilterQuery}
                  onChange={(e) => setLocationAuditFilterQuery(e.target.value)}
                  style={{
                    width: '100%',
            ...
            <div style={{ overflowX: 'auto', border: '1px solid var(--border-default)', borderRadius: 'var(--radius-sm)', maxHeight: '420px', overflowY: 'auto' }}>
              <table className="data-table" style={{ width: '100%', fontSize: '0.8rem', borderCollapse: 'collapse' }}>
                <thead style={{ position: 'sticky', top: 0, zIndex: 2, backgroundColor: 'var(--bg-surface-elevated)' }}>
                  <tr>
                    <th style={{ padding: '0.6rem 0.75rem', textAlign: 'left' }}>Hedef Lokasyon</th>
                    <th style={{ padding: '0.6rem 0.75rem', textAlign: 'left' }}>Google Kriter ID</th>
                    <th style={{ padding: '0.6rem 0.75rem', textAlign: 'center' }}>API Durumu</th>
            ...
            <div style={{
              ...
              <Info size={15} color="var(--brand-primary)" style={{ flexShrink: 0 }} />
              <span>
                <strong>Google Ads Doğrulama Notu:</strong> Google Ads API, arama sayısı gizlilik sınırının altındaki veya hedef dilde aranması olmayan küçük bölgeler için resmi olarak 0 hacim döndürür. Tablodaki veriler Google Ads Keyword Planner'dan filtrelenmeden doğrudan alınmıştır.
              </span>
            </div>
